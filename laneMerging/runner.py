...
def generate_routefile():
    ...
    pA = 1. / 30 # a vehicle is generated every 30 seconds in average.
    pB = 1. / 30 # a vehicle is generated every 30 seconds in average.
    with open("laneMerging.rou.xml", "w") as routes:
        print("""<routes>
        <vType id="typeA" type="passenger" length="5" accel="3.5" decel="5" sigma="0.0" maxSpeed="50" color="yellow"/>
        <vType id="typeB" type="passenger" length="5" accel="3.5" decel="5" sigma="0.0" maxSpeed="50" color="blue"/>
...
def compute_earliest_arrival(junction_x):
    a = np.array([0])
    b = np.array([0])
    id_a = np.array([''])
    id_b = np.array([''])
    currentTime = traci.simulation.getTime()
    print('compute time', currentTime)
    ...
    while traci.simulation.getMinExpectedNumber() > 0:  # The number of vehicles which are in the net plus the ones still waiting to start. 
        ...
        if step == period:
            if traci.lanearea.getLastStepVehicleNumber("dA") > 0 and traci.lanearea.getLastStepVehicleNumber("dB") > 0:
                a, b, id_a, id_b = compute_earliest_arrival(junction_x)
                order_stack = compute_entering_time(a, b)
                index_A = 1
                index_B = 1
                schedule_A = []
                schedule_B = []
                while len(order_stack) > 0:
                    top = order_stack.pop()
                    if top[0] == 'A':
                        schedule_A.append((id_a[index_A], top[2]))
                        ...
                        schedule_B.append((id_b[index_B], top[2]))
